...
Before any pointer or model side effect, the runner rejects an incomplete or duplicate case × arm × replicate inventory, changed input/capture hashes, a non-2-versus-3 confirmation, mismatched limits, missing qualification/calibration/holdout bytes, or an unfunded full inventory. A/A and confirmation are distinct registrations; A/A completion cannot authorize confirmation.

Each trial has a fresh `TMPDIR`. The runner fsyncs its reservation and prepared identity before pointer work, then fsyncs dispatch intent before `runtime-evaluate`. It retains complete stdout/stderr bytes, argv and exit status, the entire runtime state directory, snapshot, result or failure, DB inspection, inference accounting, and any derived text. `UNKNOWN`, ambiguous DB/admin acknowledgement, malformed outcome, or exhausted budget stops the loop without advancing another pointer. A known generation failure remains a trial and follows the exact registered no-retry continuation rule.
...
Current real confirmation is not executable: the frozen QCOM/MRVL source world still lacks Silver `unknown_price_basis_split`. No M2 A/A result or confirmation claim exists until complete eligible inputs and all registered references are supplied.
